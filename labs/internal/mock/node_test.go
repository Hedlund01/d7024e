...
func TestHelloworld(t *testing.T) {
	...
	alice, _ := NewMockNode(network, net.Address{IP: "127.0.0.1", Port: 8080})
	bob, _ := NewMockNode(network, net.Address{IP: "127.0.0.1", Port: 8081})

	// Channel for synchronization
	done := make(chan struct{})

	// Alice says hello when she receives a message
	alice.Handle("hello", func(msg net.Message, node node.INode) error {
		fmt.Printf("Alice gets: %s\n", msg.Payload)
		return msg.ReplyString("reply", "Nice to meet you!")
	})

	// Bob prints replies
	bob.Handle("reply", func(msg net.Message, node node.INode) error {
		fmt.Printf("Bob: %s\n", string(msg.Payload))
		done <- struct{}{}
		return nil
	})
...
func TestNodeCommunication(t *testing.T) {
	...
	// Create nodes
	nodeA, err := NewMockNode(network, nodeAAddr)
	if err != nil {
		t.Fatalf("Failed to create node A: %v", err)
	}

	nodeB, err := NewMockNode(network, nodeBAddr)
	if err != nil {
	...
	nodeA.Handle("ping", func(msg net.Message, node node.INode) error {
	...
	nodeA.Handle("pong", func(msg net.Message, node node.INode) error {
		content := extractContent(msg.Payload)
	...
	nodeB.Handle("ping", func(msg net.Message, node node.INode) error {
		content := extractContent(msg.Payload)
...
func TestNetworkPartitioning(t *testing.T) {
	...
	nodeB.Handle("test", func(msg net.Message, node node.INode) error {
		content := extractContent(msg.Payload)
...
func TestBroadcastPattern(t *testing.T) {
	...
	for i, addr := range addrs {
		mockNode, err := NewMockNode(network, addr)
		if err != nil {
		...
		mockNode.Handle("broadcast", func(msg net.Message, node node.INode) error {
			content := extractContent(msg.Payload)
...
func TestRequestResponsePattern(t *testing.T) {
	...
	// Create nodes
	client, err := NewMockNode(network, clientAddr)
	if err != nil {
		t.Fatalf("Failed to create client: %v", err)
	}

	server, err := NewMockNode(network, serverAddr)
	if err != nil {
	...
	server.Handle("request", func(msg net.Message, node node.INode) error {
		content := extractContent(msg.Payload)
	...
	client.Handle("response", func(msg net.Message, node node.INode) error {
		content := extractContent(msg.Payload)
